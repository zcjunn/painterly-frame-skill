...
Read [composition-color-lock.md](composition-color-lock.md) first. This file supplies the colour audit and authorship method; the lock decides whether the source colour topology is preserved, rebalanced or re-scripted. Painterly continuity may connect colours under one light, but it does not own the palette decision.
