...
description: Create or edit original painterly frames with thumbnail-visible directorial restaging, scene-owned colour authorship, focal-versus-context contrast ownership, high/mid/low-key exposure, reconstructed or expressively exaggerated graphic shapes, faceted planes, material-specific marks, spatial depth, selective 2D intervention, and model-portable multi-scale QA. Use when a photo or prompt should become a strongly stylized painted frame, abstract scene reconstruction, or finished-frame review. Do not use for faithful photo correction, pixel-locked preservation, or copying a named work, character, logo, or exact frame.
---
...
4. Compile a short priority-ordered prompt using [references/prompt-compiler.md](references/prompt-compiler.md). Before model-specific wording, write the portable render contract from [references/model-consistency.md](references/model-consistency.md): canvas and anchors; five-to-nine macro masses; three value groups; colour-role ownership; focal/support/context contrast tiers; subject plane map; at least three relevant material grammars when the scene contains three or more important materials; edge hierarchy; and anti-filter failure conditions. Explicitly reject photographic underpainting with a global paint filter when the user wants strong stylization.
